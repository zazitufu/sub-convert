...
class ConverterService:
    ...
    def process_once(self) -> list[str]:
        ...
        try:
            ...
            logger.exception("Failed to refresh rule assets; continuing with cached rules")
        ...
        for source_path in sorted(self.config.scan_dir.iterdir()):
            ...
            try:
                if self._process_source(source_path, sources):
                    processed.append(source_path.name)
            except Exception:
                logger.exception("Failed to process source %s; skipping", source_path.name)

        self.state_store.save(state)
        return processed

    def _process_source(self, source_path: Path, sources: dict) -> bool:
        ...
        if not nodes:
            logger.warning("No supported nodes found in source %s; skipping", source_name)
            return False
